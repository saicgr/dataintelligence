...
export default function Library() {
  ...
  const empty = de.length === 0 && ai.length === 0;
  return (
    <Screen>
      <View
        style={{
      ...
      <T muted size={12}>{total} senior Q&amp;A across {TRACKS.length} tracks · junior-trap vs senior-correct</T>
      {savedCount > 0 && !query && (
        <Pressable
          onPress={() => {
            startSaved();
      ...
      {empty && <T muted size={13} style={{ marginTop: 8 }}>No tracks match “{q}”.</T>}
    </Screen>
  );
}
